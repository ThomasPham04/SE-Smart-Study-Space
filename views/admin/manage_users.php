...
if (isset($_POST['delete_user'])) {
    ...
    if ($user_id != $_SESSION['user']['id']) {
        if ($admin->manageUser('delete', null, $user_id)) {
...
if (!empty($search) || !empty($filter_type)) {
    $users = array_filter($users, function($user) use ($search, $filter_type) {
        $matches_search = empty($search) || 
            stripos($user['username'], $search) !== false || 
            stripos($user['name'], $search) !== false || 
            stripos($user['email'], $search) !== false;
        
        $matches_type = empty($filter_type) || $user['user_type'] === $filter_type;
